...
// Enhanced PMID Enrichment Pipeline with Security and Live APIs
class PMIDEnrichmentPipeline {
    ...
    async enrichPMID(pmid) {
        ...
        try {
            ...
            // First, get basic article info from eSummary
            const summaryUrl = `${baseUrl}esummary.fcgi?db=pubmed&id=${pmid}&retmode=json${apiKeyParam}`;
            console.log('Fetching PMID summary:', summaryUrl);
            
            const summaryResponse = await fetch(summaryUrl);
            if (!summaryResponse.ok) {
                throw new Error(`NCBI API error: ${summaryResponse.status}`);
            }
            ...
            const articleInfo = summaryData.result[pmid];
            
            // Get detailed info from eFetch
            const fetchUrl = `${baseUrl}efetch.fcgi?db=pubmed&id=${pmid}&retmode=xml${apiKeyParam}`;
            console.log('Fetching PMID details:', fetchUrl);
            
            const fetchResponse = await fetch(fetchUrl);
            if (!fetchResponse.ok) {
                throw new Error(`NCBI eFetch error: ${fetchResponse.status}`);
            }
            ...
            // Parse XML data
            const article = xmlDoc.querySelector('PubmedArticle Article');
            if (!article) {
                throw new Error('Invalid XML response from NCBI');
            }
            ...
            const journal = article.querySelector('Journal Title')?.textContent?.trim() || 
                           article.querySelector('MedlineTA')?.textContent?.trim() || '';
            ...
            const authors = Array.from(authorNodes).map(author => {
                ...
                return lastName && initials ? `${lastName} ${initials}` : lastName;
            }).filter(Boolean).slice(0, 10).join(', ');
            
            // Extract publication date and details
            const pubDate = article.querySelector('PubDate');
            let year = '';
            if (pubDate) {
                year = pubDate.querySelector('Year')?.textContent || 
                       pubDate.querySelector('MedlineDate')?.textContent?.match(/\d{4}/)?.[0] || '';
            }
            ...
            // Extract MeSH terms
            const meshTerms = [];
            const meshNodes = xmlDoc.querySelectorAll('MeshHeading DescriptorName');
            meshNodes.forEach(node => {
                const term = node.textContent?.trim();
                if (term) {
                    meshTerms.push({ name: term, type: 'mesh' });
                }
            });
            
            // Look for clinical trial registry numbers
            let nct = '';
            const siNodes = xmlDoc.querySelectorAll('AccessionNumber');
            for (const node of siNodes) {
                const text = node.textContent;
                if (text && text.match(/^NCT\d{8}$/i)) {
                    nct = text.toUpperCase();
                    break;
                }
            }
            
            // If we have an NCT, try to get clinical trial data
            let clinicalTrial = null;
            if (nct) {
                try {
                    clinicalTrial = await this.fetchClinicalTrialData(nct);
                } catch (e) {
                    console.warn('Failed to fetch clinical trial data:', e.message);
                }
            }
            ...
            const result = {
                ...
                unified: {
                    ...
                    doi: SecurityUtils.sanitizeHtml(doi),
                    nct: SecurityUtils.sanitizeHtml(nct),
                    tags: meshTerms.slice(0, 15),
                    clinicalTrial: clinicalTrial
                }
            };
            
            console.log('PMID enrichment successful:', result);
            return result;
            
        } catch (error) {
            console.error('PMID enrichment failed:', error);
            
            // Fallback to demo data for known PMIDs
            if (pmid === '18539917') {
                return {
                    status: 'complete',
                    unified: {
                        pmid: pmid,
                        title: SecurityUtils.sanitizeHtml('Effects of intensive glucose lowering in type 2 diabetes'),
                        authors: 'Gerstein HC, Miller ME, Byington RP, Goff DC Jr, Bigger JT, Buse JB, Cushman WC, Genuth S, Ismail-Beigi F, Grimm RH Jr, Probstfield JL, Simons-Morton DG, Friedewald WT; Action to Control Cardiovascular Risk in Diabetes Study Group',
                        journal: 'N Engl J Med',
                        year: '2008', volume: '358', pages: '2545-59',
                        doi: '10.1056/NEJMoa0802743', nct: 'NCT00000620',
                        clinicalTrial: {
                            nctId: 'NCT00000620',
                        ...
                        tags: [
                            ...
                            { name: 'Glycated Hemoglobin', type: 'mesh' },
                            { name: 'Cardiovascular Diseases', type: 'mesh' },
                            { name: 'Randomized Controlled Trial', type: 'mesh' },
                            { name: 'Phase III Clinical Trial', type: 'trial' },
                            { name: 'Intensive Glucose Control', type: 'intervention' }
                        ]
                    }
                };
            }
            ...
            if (pmid === '23842776') {
                return {
                    ...
                    unified: {
                        ...
                        journal: 'Nature', year: '2014', volume: '510', pages: '363-9',
                        ...
                        tags: [
                            ...
                            { name: 'Paracrine Communication', type: 'mesh' },
                            { name: 'Cellular Heterogeneity', type: 'condition' },
                            { name: 'Genomics', type: 'mesh' }
                        ]
                    }
                };
            }
    ...
    async enrichDOI(doi) {
        ...
        try {
            ...
            if (this.crossrefEmail) {
                headers['User-Agent'] = `SilentStacks/2.0 (mailto:${this.crossrefEmail})`;
            }
            ...
            const authors = work.author?.map(author => {
                ...
                if (family && given) {
                    return `${family} ${given.split(' ').map(n => n.charAt(0)).join('')}`;
                }
                return family || given;
            }).filter(Boolean).slice(0, 10).join(', ') || '';
    ...
    async enrichNCT(nct) {
        ...
        try {
            const clinicalTrial = await this.fetchClinicalTrialData(nct);
            ...
            const result = {
                ...
                unified: {
                    ...
                    tags: [
                        ...
                        { name: clinicalTrial.condition || 'Clinical Trial', type: 'condition' }
            ...
            if (nct === 'NCT00000620') {
                return {
                    ...
                    unified: {
                        nct: nct,
                        title: SecurityUtils.sanitizeHtml('Action to Control Cardiovascular Risk in Diabetes (ACCORD)'),
                        clinicalTrial: {
                            nctId: nct,
                            title: 'Action to Control Cardiovascular Risk in Diabetes (ACCORD)',
                            phase: 'Phase 3', status: 'Completed',
    ...
    async fetchClinicalTrialData(nct) {
        try {
            // ClinicalTrials.gov API
            const ctUrl = `https://clinicaltrials.gov/api/query/full_studies?expr=${nct}&fmt=json`;
            console.log('Fetching clinical trial data:', ctUrl);
            ...
            if (!data.FullStudiesResponse || !data.FullStudiesResponse.FullStudies || 
            ...
            if (!protocolSection) {
                throw new Error('Invalid clinical trial data structure');
            }
            ...
            const phase = design?.PhaseList?.Phase?.[0] || '';
            const studyStatus = status?.OverallStatus || '';
            
            // Extract sponsor
            let sponsor = '';
            if (contacts?.LeadSponsor) {
                sponsor = contacts.LeadSponsor.LeadSponsorName || '';
            }
            
            // Extract condition
            let condition = '';
            if (protocolSection.ConditionsModule?.ConditionList) {
                condition = protocolSection.ConditionsModule.ConditionList.Condition?.[0] || '';
            }
            
            return {
                nctId: nct,
                title: SecurityUtils.sanitizeHtml(title),
                phase: SecurityUtils.sanitizeHtml(phase),
                status: SecurityUtils.sanitizeHtml(studyStatus),
                sponsor: SecurityUtils.sanitizeHtml(sponsor),
                condition: SecurityUtils.sanitizeHtml(condition)
            };
            
        } catch (error) {
            console.error('Clinical trial data fetch failed:', error);
            throw error;
        }
    }
}
        ...
        tags.slice(0, 8).forEach(tag => {
            ...
            if (tagType === 'mesh') {
                ...
                if (tagLower.includes('phase') || /phase\s+[ivx]+/i.test(tagName)) {
                    ...
                    tagType = 'trial';
                } else if (tagLower.includes('carcinoma') || tagLower.includes('disease') || tagLower.includes('syndrome') || tagLower.includes('diabetes')) {
                    tagType = 'condition';
                } else if (tagLower.includes('drug') || tagLower.includes('therapy') || tagLower.includes('treatment') || tagLower.includes('control')) {
                    tagType = 'intervention';
                } else if (tagLower.includes('pharmaceuticals') || tagLower.includes('company') || tagLower.includes('inc') || tagLower.includes('institute')) {
                    tagType = 'sponsor';
                }
            }
...
function sortTable(field) {
    ...
    renderRequests();
    announceToScreenReader(`Table sorted by ${cleanField} ${APP_STATE.currentSort.direction === 'asc' ? 'ascending' : 'descending'}`);
}
...
function sortRequests(requests) {
    ...
    return [...requests].sort((a, b) => {
        ...
        if (field === 'created' || field === 'updated') {
            ...
            // String comparison - handle null/undefined values
...
function renderTableView(requests) {
    ...
    requests.forEach(request => {
        ...
        if (request.doi) {
            const doiLink = `<a href="https://doi.org/${SecurityUtils.sanitizeHtml(request.doi)}" target="_blank" rel="noopener noreferrer" title="View article via DOI">${citation}</a>`;
...
function renderCardsView(requests) {
    ...
    requests.forEach(request => {
        ...
        // Create clickable title with DOI if available
        let titleDisplay = SecurityUtils.sanitizeHtml(request.title || 'Untitled');
        if (request.doi) {
            titleDisplay = `<a href="https://doi.org/${SecurityUtils.sanitizeHtml(request.doi)}" target="_blank" rel="noopener noreferrer" style="color: inherit; text-decoration: none;" title="View article via DOI">${titleDisplay}</a>`;
        }
...
function exportToNLM() {
    ...
    const nlmCitations = APP_STATE.requests.map(req => {
        ...
        // Authors in NLM format (Last FM, Last FM, Last FM)
        if (req.authors && req.authors !== 'N/A') {
            let authorString = req.authors;
            
            // Handle different author formats and convert to NLM
            if (authorString.includes(';')) {
                // Handle semicolon-separated authors
                const authorList = authorString.split(';').map(author => {
                    const cleanAuthor = author.trim();
                    // If already in Last FM format, keep it
                    ...
                    if (parts.length >= 2) {
                        ...
                        if (middle) {
                            initials += middle.split(' ').map(m => m.charAt(0).toUpperCase()).join('');
                        }
                        return `${last} ${initials}`;
                    }
                    return cleanAuthor;
                }).slice(0, 6); // NLM typically lists up to 6 authors
        ...
        // Title in sentence case, ends with period
        if (req.title && req.title !== 'N/A' && req.title !== 'Untitled') {
            let title = req.title.trim();
            // Convert to sentence case (first letter capitalized, rest lowercase except proper nouns)
            title = title.charAt(0).toUpperCase() + title.slice(1).toLowerCase();
            if (!title.match(/[.!?]$/)) title += '.';
            citation += title + ' ';
        }
        
        // Journal abbreviation (in NLM format)
        if (req.journal && req.journal !== 'N/A') {
            let journal = req.journal;
            // Apply common NLM abbreviations
        ...
        // Date and volume information in NLM format: YYYY Mon DD;Volume(Issue):Pages.
        let dateVolumePages = '';
