...
(function(){
  ...
  const APIs = {
    ...
    async elinkNCT(pmid){
      ...
      const t = await r.text(); const m = t.match(/NCT\d{8}/i); return m?m[0].toUpperCase():null;
    },
    ...
    async ctgovFields(nct){
      ...
      const url = `https://clinicaltrials.gov/api/query/study_fields?expr=${encodeURIComponent(nct)}&min_rnk=1&max_rnk=1&fields=${encodeURIComponent(fields)}&fmt=json`;
      const r = await fetch(url, { method:'GET' }); if(!r.ok) throw new Error(`ctgov fields ${r.status}`);
      return await r.json();
    },
    // Fallback 2: v1 full_studies
    async ctgovFull(nct){
      const url = `https://clinicaltrials.gov/api/query/full_studies?expr=${encodeURIComponent(nct)}&min_rnk=1&max_rnk=1&fmt=json`;
  ...
  const Extract = {
    ...
    nct(summary, xml){
      const scan = (s)=>{ if(!s) return null; const m = String(s).match(/NCT\d{8}/i); return m?m[0].toUpperCase():null; };
      // DataBank
  ...
  function parseCtgovV2(data, nct){
    ...
    return {
      ...
      studyUrl: `https://clinicaltrials.gov/study/${nct}`, apiVersion:'v2'
    };
  }
  ...
  function parseCtgovFields(json, nct){
    ...
    return {
      ...
      studyUrl: `https://clinicaltrials.gov/study/${nct}`, apiVersion:'v1-fields'
    };
  }
  ...
  function parseCtgovFull(json, nct){
    ...
    const out = {
      ...
      studyUrl: `https://clinicaltrials.gov/ct2/show/${nct}`, apiVersion:'v1-full'
    };
    return out;
  }
