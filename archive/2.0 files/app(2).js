...
/*! SilentStacks app.js (readable) - v2.2.0
 *  Mirrors app.min.js exactly (functionality parity), but with spacing and comments
 *  - CT.gov v2 API with v1 fallback
 *  - Robust DOI decode & normalize (hex/dec HTML entities, URL encodings)
...
(function () {
  ...
  function decodeHTMLEntities(str) {
    if (!str) return "";
    // hex entities: &#xNN;
    str = str.replace(/&#x([0-9a-f]+);/gi, (_, h) =>
  ...
  function normalizeDOI(input) {
    return decodeDOI(input).toLowerCase();
  }
  ...
  class PMIDEnrichmentPipeline {
    ...
    async enrichPMID(pmid) {
      ...
      try {
        ...
        if (out.pubmed.nct) {
          try {
            out.clinicalTrial = await this.fetchEnhancedClinicalTrialData(
              out.pubmed.nct
            );
    ...
    async fetchPubMedXML(pmid, keyParam) {
      ...
      const res = await fetch(url);
      if (!res.ok)
        throw new Error("PubMed XML failed: " + res.status + " " + res.statusText);
    ...
    async extractNCTFromAll(summaryData, xmlDoc, pmid) {
      ...
      // 3) Text search in title + abstract
      const title = summaryData.title || "";
      const absText = Array.from(xmlDoc.querySelectorAll("AbstractText"))
        .map((n) => n.textContent || "")
        .join(" ");
      ...
      // 4) ELink dbfrom=pubmed -> clinicaltrials
    ...
    async fetchNCTViaELink(pmid) {
      try {
        await limit("pubmed");
        const url =
          "https://eutils.ncbi.nlm.nih.gov/entrez/eutils/elink.fcgi?dbfrom=pubmed&db=clinicaltrials&id=" +
          pmid +
          "&tool=SilentStacks&email=contact@example.com";
        const res = await fetch(url);
    ...
    async fetchEnhancedClinicalTrialData(nctId) {
      ...
      throw new Error(
        "CT.gov lookup failed (v2 & v1). This can be a CORS limitation in the browser or a network block."
      );
    }
      ...
      const paths = [
        ...
        "ProtocolSection.OutcomesModule." +
          (field === "primaryOutcomes"
            ...
            : "SecondaryOutcomeList.SecondaryOutcome"),
      ];
      for (const p of paths) {
        const arr = this.getNestedValue(study, p);
        if (Array.isArray(arr)) {
    ...
    mergeData(pubmed, ct) {
      ...
      if (ct) {
        ...
        if (ct.sponsorClass && tags.length < 8)
          tags.push({
            name: ct.sponsorClass,
            type: "sponsor",
            source: "clinicaltrials",
            color: "#e5e7eb",
          });
    ...
    extractMeshFromXML(xml) {
      ...
      nodes.forEach((n) => {
        ...
        if (t && list.length < 8) list.push(t);
      });
      return list;
    }
  ...
  function addEnrichedTags(tags) {
    ...
    if (!box) return;
    Array.from(box.querySelectorAll('.tag[data-auto="true"]')).forEach((e) =>
      e.remove()
    );
    tags.forEach((t) => {
  ...
  function updateTagChips() {
    ...
    // Remove manual chips (keep auto chips with data-auto="true")
    container
      .querySelectorAll('.tag:not([data-auto="true"])')
      .forEach((n) => n.remove());
    ...
    set.forEach((name) => {
      const autoDup = Array.from(
        container.querySelectorAll('.tag[data-auto="true"]')
      ).some((t) => t.textContent === name);
      if (autoDup) return;
  ...
  function showMetadataStatus(unified) {
    ...
    if (!wrap) {
      ...
      wrap.style.cssText =
        "background:linear-gradient(135deg,#ecfdf5,#d1fae5);border:2px solid #10b981;border-radius:12px;padding:16px;margin:16px 0;font-size:.9rem;";
    ...
    let html =
      '<div style="font-weight:700;color:#059669;margin-bottom:8px;">📊 METADATA ENRICHMENT STATUS</div>';
    if (mesh.length)
      html +=
        "<div><strong>✅ MeSH Headings:</strong> " +
        mesh.length +
        " terms extracted (" +
        mesh.map((t) => t.name).join(", ") +
        ")</div>";
    if (unified.clinicalTrial) {
      const ct = unified.clinicalTrial;
      html +=
        "<div><strong>✅ Clinical Trial:</strong> " +
        (unified.nct || ct.nctId) +
        " linked</div>";
  ...
  function populateFormWithEnrichedData(unified) {
    ...
    if (nctField && unified.nct) nctField.value = unified.nct;
    ...
    if (unified.clinicalTrial || (unified.tags || []).some((t) => t.type === "mesh")) {
      showMetadataStatus(unified);
    }
  }
  ...
  function showEnhancedNCTSidebar(ct) {
    ...
    let html =
      '<div class="nct-header"><div class="nct-id">🧪 ' +
      (ct.nctId || "") +
      "</div>" +
      (ct.phase ? '<div class="nct-phase">Phase ' + ct.phase + "</div>" : "") +
      "</div>";

    if (ct.title) {
      html +=
        '<div style="background:#f0f9ff;border-radius:12px;padding:16px;margin:16px 0;border-left:4px solid #3b82f6;">' +
        '<div style="font-size:.75rem;font-weight:600;color:#1e40af;text-transform:uppercase;margin-bottom:6px;">STUDY TITLE</div>' +
        '<div style="font-size:.9rem;font-weight:600;color:#1f2937;line-height:1.4;">' +
        ct.title +
        "</div></div>";
    }
    ...
    function row(label, value) {
      if (!value) return "";
      return (
        '<div class="nct-detail-row"><div class="nct-detail-label">' +
        label +
        '</div><div class="nct-detail-value"><strong>' +
        value +
        "</strong></div></div>"
      );
    }
    ...
    if (ct.enrollmentCount)
      html += row(
        "Enrollment",
        String(ct.enrollmentCount) + (ct.enrollmentType ? " (" + ct.enrollmentType + ")" : "")
      );
        ...
        html +=
          ...
          (iv.description
            ? '<div style="font-size:.8rem;color:#6b7280;margin-top:2px;">' +
              String(iv.description).substring(0, 100) +
              (String(iv.description).length > 100 ? "..." : "") +
              "</div>"
            : "") +
          "</div>";
      });
    ...
    if (Array.isArray(ct.primaryOutcomes) && ct.primaryOutcomes.length) {
      ...
      ct.primaryOutcomes.slice(0, 3).forEach((o, i) => {
        html +=
          ...
          (i > 0 ? "border-top:1px solid rgba(239,68,68,.1);padding-top:8px;" : "") +
          '">' +
          '<div style="font-weight:600;font-size:.85rem;color:#1f2937;margin-bottom:4px;">' +
          (o.measure || "Primary Outcome") +
          "</div>" +
          (o.timeFrame
            ? '<div style="font-size:.8rem;color:#6b7280;"><strong>Time Frame:</strong> ' +
              o.timeFrame +
              "</div>"
            : "") +
          (o.description
            ? '<div style="font-size:.8rem;color:#6b7280;margin-top:4px;">' +
              String(o.description).substring(0, 150) +
              (String(o.description).length > 150 ? "..." : "") +
              "</div>"
            : "") +
          "</div>";
      });
    ...
    if (Array.isArray(ct.secondaryOutcomes) && ct.secondaryOutcomes.length) {
      html +=
        '<div style="background:#eef2ff;border-radius:12px;padding:16px;margin:16px 0;border-left:4px solid #6366f1;">' +
      ...
      ct.secondaryOutcomes.slice(0, 3).forEach((o, i) => {
        html +=
          '<div style="margin-bottom:12px;' +
          (i > 0 ? "border-top:1px solid rgba(99,102,241,.15);padding-top:8px;" : "") +
          '">' +
          '<div style="font-weight:600;font-size:.85rem;color:#1f2937;margin-bottom:4px;">' +
          (o.measure || "Secondary Outcome") +
          "</div>" +
          (o.timeFrame
            ? '<div style="font-size:.8rem;color:#6b7280;"><strong>Time Frame:</strong> ' +
              o.timeFrame +
  ...
  function clearForm() {
    ["title","authors","journal","year","doi","nct","tags","patron-email","notes"].forEach(id => {
      if (q(id)) q(id).value = "";
    });
  ...
  async function enhancedPMIDLookup() {
    ...
    if (!/^\d{6,9}$/.test(pmid)) {
      setStatus("PMID must be 6-9 digits", "error");
      return;
    }
    ...
    if (pmid === "18539917" || pmid === "23842776") {
      ...
      await sleep(800);
      const demo =
        window.SilentStacks && window.SilentStacks._demoData
          ? window.SilentStacks._demoData(pmid)
          : null;
      if (demo) {
    ...
    const settings = JSON.parse(localStorage.getItem("silentstacks_settings") || "{}");
    const pipe = new PMIDEnrichmentPipeline({
    ...
    try {
      ...
      if (result.status === "complete" && result.unified) {
        ...
        if (result.unified.clinicalTrial) {
          ok.push("🧪 Clinical trial " + (result.unified.nct || "") + " analyzed");
          showEnhancedNCTSidebar(result.unified.clinicalTrial);
        }
        if ((result.unified.tags || []).length > 0)
          ok.push("🏷️ " + result.unified.tags.length + " tags added");
      ...
      if ((e.message || "").includes("Network") || (e.message || "").includes("fetch")) {
        setStatus(
          "⚠️ CT.gov/PubMed calls blocked by network/CORS/SW. Try demo PMIDs 18539917 / 23842776 or run on an https origin.",
          "warning"
        );
      } else {
  ...
  document.addEventListener("DOMContentLoaded", function () {
    // Buttons
    const btn = q("lookup-pmid");
    if (btn) btn.addEventListener("click", enhancedPMIDLookup);
    ...
    const savePaperBtn = q("save-paper");
    if (savePaperBtn) savePaperBtn.addEventListener("click", savePaper);

    // Inputs
    const tagsInput = q("tags");
    if (tagsInput) tagsInput.addEventListener("input", updateTagChips);
    ...
    if (pmidInput) {
      ...
      pmidInput.addEventListener("input", function () {
    ...
    try {
      console.log("🧬 SilentStacks app.js (readable) v2.2 initialized");
    } catch (_) {}
  });

  // Expose API (useful for unit/integration tests)
    ...
    _demoData: function (pmid) {
      if (pmid === "18539917") {
        return {
          ...
          title:
            "Sorafenib in patients with advanced hepatocellular carcinoma: a randomized, double-blind, placebo-controlled phase 3 trial",
          authors:
            "Llovet JM; Ricci S; Mazzaferro V; Hilgard P; Gane E; Blanc JF; de Oliveira AC; Santoro A; Raoul JL; Forner A; Schwartz M; Porta C; Zeuzem S; Bolondi L; Greten TF; Galle PR; Seitz JF; Borbath I; Häussinger D; Giannaris T; Shan M; Moscovici M; Voliotis D; Bruix J",
          ...
          clinicalTrial: {
            ...
            secondaryOutcomes: [
              { measure: "Progression-Free Survival", timeFrame: "Up to 5 years" },
              { measure: "Time to Progression", timeFrame: "Up to 5 years" },
            ],
            eligibility: {
              criteria: "Advanced HCC not amenable to resection",
              gender: "All",
              minimumAge: "18 Years",
              maximumAge: "N/A",
