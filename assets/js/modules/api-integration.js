// Enhanced API Integration for SilentStacks v1.4 - Modular Architecture (FIXED)
...
(function() {
    ...
    window.SilentStacksAPI = (() => {
        ...
        class APIQueue {
            ...
            static saveQueue() {
                ...
                if (typeof localStorage === 'undefined') {
                    console.warn('localStorage not available, queue not persisted');
                    return;
                }
            ...
            static loadQueue() {
                // Check if localStorage is available
                if (typeof localStorage === 'undefined') {
                    console.warn('localStorage not available, cannot load queue');
                    this.queue = [];
                    return;
                }
        ...
        class PubMedAPI {
            static async fetchPubMed(pmid) {
                ...
                try {
                    ...
                    const apiKey = window.SilentStacks?.settings?.apiKey || 
                                  window.settings?.apiKey || 
                                  '';
                    ...
                    if (parserError) {
                        ...
                        // Return basic metadata if XML parsing fails
                        return meta;
                    }
        ...
        // Initialize on load (fixed race condition)
    ...
    })(); // Properly close the SilentStacksAPI module
        ...
        displayMeSHTerms(meshHeadings) {
            ...
            if (meshSection && meshTags) {
                ...
                const meshHTML = meshHeadings.map(mesh => {
                    // Properly escape the term for use in onclick
                    const escapedTerm = mesh.term.replace(/'/g, "\\'").replace(/"/g, "&quot;");
                    return `
                        <button type="button" class="mesh-tag" onclick="window.addMeshToTags('${escapedTerm}')" title="${mesh.isMajorTopic ? 'Major Topic' : 'Minor Topic'}">
    ...
    // Add the missing global function for MeSH tag clicking
    window.addMeshToTags = function(term) {
        const tagsInput = document.getElementById('tags');
        if (tagsInput) {
            const currentTags = tagsInput.value.trim();
            // Check if term already exists
            if (currentTags && !currentTags.includes(term)) {
                tagsInput.value = `${currentTags}, ${term}`;
            } else if (!currentTags) {
                tagsInput.value = term;
            }
    ...
    console.log('✅ SilentStacksAPI module registered');
    console.log('✅ APIIntegration module registered');
    console.log('✅ MeSH tag handler registered');

})(); // Close the entire wrapper IIFE
